...
public class ReplaceInnerOpen implements ZetaUDF {

    @Override
    public String functionName() {
        return "REPLACE_INNEROPEN";
    }
    ...
    public Object evaluate(List<Object> args) {
        // REPLACE_INNEROPEN('{replace}','{arg2}',{from},{end})
        ...
        if (StrUtil.isNotEmpty(replace) && StrUtil.isNotEmpty(data)) {
            // 遮盖前n
            String replaceFrom = StrUtil.replace(data, 0, before, replace);
            // 遮盖后m
            data =
                    StrUtil.replace(
                            replaceFrom,
                            replaceFrom.length() - after,
                            replaceFrom.length(),
                            replace);
        }
        return data;
